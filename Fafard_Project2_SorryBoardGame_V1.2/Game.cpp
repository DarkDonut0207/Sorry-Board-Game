...
Game::Game() {
    ...
    hashFile.open("playerHashTable.txt");
    while(!recWinF.eof()) {
...
Game::~Game() {
    ...
    recWinF.open ("recentWinners.txt", ofstream::trunc);
    hashFile.open("playerHashTable.txt", ofstream::trunc);
    while (!winList.empty()) {
...
// A hash table to search to estimate if someone has played before
void Game::hashPlayer(string plyr, bool check) {
    if (check) {
        if (hashTble[RSHash(plyr)%10000] == '1' && hashTble[JSHash(plyr)%10000] == '1') {
            cout << "\nUser has (likely) played before.\n";
        } else cout << "\nUser has not played before.\n";
    } else {
        hashTble[RSHash(plyr)%10000] = '1';
        hashTble[JSHash(plyr)%10000] = '1';
    }
    
}
// RSHash Function Definition
unsigned int Game::RSHash(const string& str) {
    // Initialize variables
    unsigned int b = 378551;
    unsigned int a = 63689;
    unsigned int hash = 0;
    for (int i=0; i<str.length(); i++) { // Loop through given string
        hash = hash * a + str[i]; // Compute hash
        a = a * b;
    }
    return hash; // Return value
}
// JSHash Function Definition
unsigned int Game::JSHash(const string& str) {
    // Initialize variables
    unsigned int hash = 1315423911;
    for(int i=0; i<str.length(); i++) {
        hash ^= ((hash << 5) + str[i] + (hash >> 2)); // Compute hash
    }
    return hash; // Return value
}
void Game::getPlayers() {
    ...
    hashPlayer(tempName, true);
    hashPlayer(tempName, false);
    for (int i=1; i<4; i++) {
